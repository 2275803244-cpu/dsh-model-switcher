			...
			return jsxs("div", {
				...
				children: [
					...
					open ? jsx("div", { className: styles.panel, style: panelPos, children: [
						...
						groups.length > 0 ? jsx("div", { className: styles.list, children: groups.map((group) => jsx("div", {
							className: styles.group,
							children: [
								jsx("div", { className: styles.groupName, children: [group.name, " ", jsx("span", { className: styles.groupHint, children: group.id })] }),
								...
								state && state.failures && state.failures.length > 0 ? jsx("div", { className: styles.err, children: state.failures.map((f) => f.name + " 加载失败：" + f.message).join("\n") }) : null
							]
						}, group.id)) }) : (state && state.status !== "loading" ? jsx("div", { className: styles.empty, children: "暂无可用模型——请先在 设置 → 模型 中配置服务商" }) : null)
					] }) : null
			]
		});
		}
