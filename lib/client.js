...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var styles = {
			...
			"fmsRoot": "_ms_fmsRoot",
			"fmsChip": "_ms_fmsChip",
			"fmsPopover": "_ms_fmsPopover",
			"fmsList": "_ms_fmsList",
			"fmsRow": "_ms_fmsRow",
			"fmsActive": "_ms_fmsActive",
			"fmsName": "_ms_fmsName",
			"fmsId": "_ms_fmsId",
			"fmsTick": "_ms_fmsTick",
			"fmsEmpty": "_ms_fmsEmpty"
		};
		//#endregion
		...
		/**
		 * Compact model selector for the composer input bar's right side.
		 * Shows the current model; clicking opens a popover with the current
		 * provider's models. Select a model to switch — provider stays the same.
		 * For provider switching, use the header panel.
		 */
		function FastModelSwitch({ selectModel, subscribe, getSnapshot, useSession }) {
			if (!subscribe || !getSnapshot) return null;
			const [open, setOpen] = react.useState(false);
			const [selecting, setSelecting] = react.useState(null);
			const rootRef = react.useRef(null);
			...
			const current = state && state.current ? state.current : null;
			const groups = state && state.groups ? state.groups : [];
			const activeGroup = current ? groups.find((g) => g.id === current.provider) : groups[0];
			react.useEffect(() => {
				if (!open) return;
				const closeOutside = (event) => {
					if (event.target instanceof Node && !rootRef.current.contains(event.target)) setOpen(false);
			...
			return jsxs("div", { ref: rootRef, className: styles.fmsRoot, children: [
				jsx("button", { type: "button", className: styles.fmsChip, onClick: () => setOpen(!open), children: label }),
				open ? jsx("div", { className: styles.fmsPopover, children: [
					activeGroup && activeGroup.models.length > 0
						? jsx("div", { className: styles.fmsList, children: activeGroup.models.map((model) => {
							const active = !!current && current.provider === activeGroup.id && current.model === model.id;
							const busy = selecting === activeGroup.id + "/" + model.id;
							return jsx("button", {
								type: "button",
								className: styles.fmsRow + (active ? " " + styles.fmsActive : ""),
								disabled: selecting !== null,
								onClick: () => pick(activeGroup.id, model.id),
								children: [
									jsx("span", { className: styles.fmsName, children: model.name }),
									jsx("span", { className: styles.fmsId, children: model.id }),
									active ? jsx("span", { className: styles.fmsTick, children: "✓" }) : (busy ? jsx("span", { className: styles.fmsTick, children: "…" }) : null)
								]
							}, activeGroup.id + "/" + model.id);
						}) })
						: jsx("div", { className: styles.fmsEmpty, children: "暂无模型" })
				] }) : null
			]});
		}
		/**
